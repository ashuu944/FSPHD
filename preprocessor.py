...
# ====================================================================================================#
# Normalization Method For Timeseries Data
# ====================================================================================================#
def normalize(country, data, var_type, method):
    log(country, f"{var_type} variables to Normalize: {list(data.columns)}")
    for v in var_type:
        # Get the columns that match the substring
        columns_to_scale = [col for col in data.columns if col.startswith(v)]
        
        if len(columns_to_scale) > 0:
            log(country, f"Columns to scale: {columns_to_scale}")
            df = data[columns_to_scale]
            
            # Choose the scaler based on the method parameter
            if method == 'standard':
                scaler = StandardScaler()
            elif method == 'min-max':
                ...
                raise ValueError("Invalid method. Choose from 'standard', 'min-max', or 'robust'.")
            ...
            log(country, f"Applied {method} scaling to columns: {columns_to_scale}")

    return data

#============================================================================================================
#           This is for normalizing structure Data
#===========================================================================================================

def normalize_structure_data(data, columns, method='standard'):
    scaler = None
    if method == 'standard':
        scaler = StandardScaler()
    elif method == 'min-max':
...
def processed_timeseries(country, data_rep, algorithm, data_rep_columns):
     ...
     data_aggregation_timeseries = normalize(country,data_aggregation_timeseries, conf.vars_timeseries,method='min-max')# Normalize variables by mean/STD data_X_timeseries = np.array(
...
def processed_conjunctural_structural(country, data_rep, algorithm, data_rep_columns):
    ...
    data_temp = pd.read_excel(os.path.join(conf.DATA_DIRECTORY, country, conf.VAR_DIRECTORY, "data_world_bank.xlsx"))
    data_aggregation = pd.merge(data_aggregation, data_temp, how="left", on=[conf.TEMPORAL_GRANULARITY[country]])
    ...
    data_aggregation = normalize_structure_data(data_aggregation, column_to_normalize, 'robust')
    
    log(country, f"Columns shape {len(column_to_normalize)}: {column_to_normalize}")
